...
const FormSection = React.forwardRef<React.ElementRef<typeof View>, FormSectionProps>(
  (
    ...
    return (
      <View className={cn('relative', Platform.OS !== 'ios' && 'flex-row gap-4', rootClassName)}>
        ...
        <View className="flex-1">
          <View
            ref={ref}
            className={cn('ios:rounded-lg ios:bg-background ios:gap-0 ios:pl-1 gap-4', className)}
            style={style}
            children={children}
            {...props}
          />
